...
void DrawInit()
{
    ...
    while (!基址 && !stopFlag)
    {
        ...
        if (pid != 0 && pid != -1)
        {
            ...
            libtersafe_base = getModuleBase("libtersafe.so:bss");
...
void 解密算法() {
    ...
    if(dating == 1) {
    if(sss == 0) {
        for (uint64_t addr = 0x7FFFFFF, i = 0, a = 0 ; addr >= 0x6500000;addr--) {
        uint64_t pd = addr * 0x1000 + 0x108;
        int pd1 = driver->read<int>(pd - 0x54);
        int pd2 = driver->read<int>(pd - 0x50);
        int pd3 = driver->read<int>(pd - 0x58);
        if (pd1 == 16777473 && pd2 == 255 && pd3 >= -1000 && pd3 <= 1000) {
        zuobiao[i] = pd;
      //  printf("坐标%lx\n",zuobiao[i]);
            i++;
            if (i == 5) {
             ...
             long 红)
{
    ...
    if (id == rc_xxl_id)
    {
        ...
        // 绘制最大血量条填充
        暴露视野不绘 ? void(0) : Draw->AddRectFilled(ImVec2((int)map_coord.X - 20 - bar_width / 2 + corner_radius - (-map_coord.H * 0.65f), (int)map_coord.Y + corner_radius - (map_coord.H * 1.1f)), ImVec2((int)map_coord.X - 20 + bar_width / 2 - corner_radius - (-map_coord.H * 0.65f), (int)map_coord.Y + bar_height - corner_radius - (map_coord.H * 1.2f)), Paint_white, corner_radius);
        ...
        // 绘制当前血量条填充
        暴露视野不绘 ? void(0) : Draw->AddRectFilled(ImVec2((int)map_coord.X - 20 - bar_width / 2 + corner_radius - (-map_coord.H * 0.65f), current_hp_fill_start_y), ImVec2((int)map_coord.X - 20 + bar_width / 2 - corner_radius - (-map_coord.H * 0.65f), (int)map_coord.Y + bar_height - (map_coord.H * 1.2f)), Paint_green, corner_radius);
    }
    else
    {
...
void DrawPlayer()
{
    ...
    long temp = ReadValue(lil2cpp_base + 0x519640); // 8F547A8
    // 矩阵数据cb
    ...
    if (/*  ReadDword(libGame_base + 0x1960C4) == 1 */    1  )
    {   // 对局判断
        // 开局数据已结束：:
        /*兵线: 0x18CE40
        野怪: 0x345E8
        ...
        long temp1 = ReadValue(libGame_base + 0x16C420);
        //  long temp21 = ReadValue(libGame_base + 0x161910);
        // 坐标数据//
        // printf("temp1 %d \n",temp12);
        long bingxiang8 = ReadValue(ReadValue(temp1 + 0x165410) + 0x4D8);


        int 自身阵营 = driver->read<int>(bingxiang8 + 0x3C);
        long bingxiang1 = temp1 + 0x238;

        //  bingxiang6[6] =bingxiang8;
      ...
      0x40 -> + 0x170 -> + 0x138
        ...
        /*   for (int x = 0, p = 0; x < 800; x++) {
       long bingxiang7 = ReadValue(bingxiang1 + x * 0x1);
       int renwu = driver->read<int>(bingxiang7 + 0x3C);
       int rwid = driver->read<int>(bingxiang7 + 0x30);
       if(p == 5) {
       break;
       }
       if(renwu == foeComp && rwid >= 105 && rwid <= 584) {
           bingxiang6[p] = bingxiang7;
         //  dataTable.heroTemp[p].Id = rwid;
           p++;
       }
   }
        ...
        for (int i = 0; i < 12; i++)
        {
         ...
         bingxiang6[i] =ReadValue(ReadValue(bingxiang1) + i * 0x18);
            ...
            0x500);*/
                ...
                {
                    zuobiao2 = ReadValue(ReadValue(ReadValue(ReadValue(bingxiang6[i] + 0x248) + 0x10) + 0x0) + 0x60); // 0xc  0x14

                    float 坐标x = (float)driver->read<int>(zuobiao2 + 0x0);
                    float 坐标y = (float)driver->read<int>(zuobiao2 + 0x8);
                ...
                if (zhengxing == foeComp)
                {
                    ...
                    dataTable.heroTemp[i].HC = driver->read<int>(
                        ReadValue(ReadValue(ReadValue(bingxiang6[i] + 0x150) + 0x168) +
                                  0x168) +
                        0x20);
                    long 蓝方敌人视野判断 = driver->read<int>(ReadValue(ReadValue(bingxiang6[i] + 0x260) + 0x68) + 0x38);
                    long 红方敌人视野判断 = driver->read<int>(ReadValue(ReadValue(bingxiang6[i] + 0x260) + 0x68) + 0x10);
                    if (zhengxing == 2)
                    {
                        视野 = driver->read<int>(
            ...
            if (坐标x && 坐标y) {
              ...
              dataTable.heroTemp[i].coord.Y = (float)坐标y;
                    ...
                    float 坐标x, 坐标y;
                    long zuobiao = ReadValue(ReadValue(ReadValue(ReadValue(bingxiang6[i] + 0x248) + 0x10) + 0x0) + 0x60); // 0xc  0x14
                    /*if (jmm == 0 || dating == 0) {
                     zhenshi = ReadValue(ReadValue(ReadValue(ReadValue(bingxiang6[i] + 0x248)+0x10)+0x0)+0x10);  //0xc  0x14
                        坐标x = (float)driver->read<int>(zhenshi + 0x0);
                        坐标y = (float)driver->read<int>(zhenshi + 0x8);
                    } else {*/
                    坐标x = (float)driver->read<int>(zuobiao + 0x0);
                    坐标y = (float)driver->read<int>(zuobiao + 0x8);
                    //}
                    if (坐标x && 坐标y)
                    {
          ...
          */
                    ...
                      if (aa > 0 || dataTable.heroTemp[i].Id == 183)
                    {
                        ...
                        if (((float)dataTable.heroTemp[i].Hp /
                             (float)dataTable.heroTemp[i].MaxHp) <= zhanshaz)
                        {
                            健康血量 = false;
                        }
                        if (方框)
            ...
            dataTable.heroTemp[i].Space =
                ...
                8192000;*/
                        ...
                        dataTable.heroTemp[i].Space2 = ReadDword(ReadValue(
                                                                     ReadValue(ReadValue(bingxiang6[i] + 0x150) + 0xF0) + 0xF8) +
                                                                 0x3C) /
                                                       8192000;
                        dataTable.heroTemp[i].Space2id = dataTable.heroTemp[i].Id * 100 + 20;
                        二技能判断 = driver->read<int>(driver->read<uint64_t>(driver->read<uint64_t>(driver->read<uint64_t>(bingxiang6[i] + 0x150) + 0xF0) + 0xF8) + 0x70);
                        // 大招
                        ...
                        if (方框技能)
                        {
                            ...
                            else
                            {
                                if (视野方框)
                                {
                                    ...
                                    else if (intvalue <= 0 && 三技能判断 != 4 && 三技能判断 != 260)
                                    {
                                    ...
                                    else if (intvalue2 <= 0 && 二技能判断 != 4 && 二技能判断 != 260)
                                    {
                                    ...
                                    else if (intvalue3 <= 0 && 一技能判断 != 4 && 一技能判断 != 260)
                                    {
            ...
            if (intvalue1_ > 0) { // 绘制召唤计时
              ...
              ImGui::GetForegroundDrawList()->AddText(
                        ...
                        if (shared)
                        {
                            ...
                            {
                                character += std::to_string(dataTable.heroTemp[i].Id) + "," + std::to_string(dataTable.heroTemp[i].Hp) + "," + std::to_string(dataTable.heroTemp[i].MaxHp) + "," + std::to_string(dataTable.heroTemp[i].Space) + "," + std::to_string(dataTable.heroTemp[i].Skill) + "," + std::to_string(((int)dataTable.DynamicData[i].coord.X)) + "," + std::to_string(((int)dataTable.DynamicData[i].coord.Y)) + "," + std::to_string((int)hp1) + "," + std::to_string(zhengxing) + "," + std::to_string(isGames > 0 ? 1 : 2) + "," + std::to_string(dataTable.heroTemp[i].HC) + "," + std::to_string(0) + "," + std::to_string(dataTable.heroTemp[i].Spaceid) + "==";
                            }
                        }
        ...
        if (野怪)
        {
            ...
            long temp2 = ReadValue(libGame_base + 0x1F68);
            BuffAddress =
                ReadValue(ReadValue(ReadValue(temp2 + 0x438) + 0x88) + 0x140) + 0x0;
            // printf("START\n");
            for (int i = 0; i < 23; i++)
            {
                ...
                if (实体指针 == 0)
                {
                    ...
                    dataTable.pve[i].X = (uint64_t)(dataTable.pve[i].cc + 0x2b8);            // 固定X坐标地址
                    dataTable.pve[i].Y = (uint64_t)(dataTable.pve[i].cc + 0x2b8 + 8);        // 固定Y坐标地址
                    dataTable.pve[i].hp = (uint64_t)(dataTable.pve[i].cc + 0x168 + 0xA8);    // 固定HP地址
                    dataTable.pve[i].maxhp = (uint64_t)(dataTable.pve[i].cc + 0x168 + 0xB0); // 固定最大HP地址
                }
                else
                {
                    dataTable.pve[i].X = ReadValue(ReadValue(实体指针 + 0x230) + 0x60) + 0x0;
                    dataTable.pve[i].Y = ReadValue(ReadValue(实体指针 + 0x230) + 0x60) + 0x8;
                    dataTable.pve[i].hp = ReadValue(实体指针 + 0x168) + 0xA8;
                    dataTable.pve[i].maxhp = ReadValue(实体指针 + 0x168) + 0xB0;
                }

                int 惩戒伤害 = driver->read<int>(
                    driver->read<uint64_t>(
                        driver->read<uint64_t>(
                            driver->read<uint64_t>(
                                driver->read<uint64_t>(
                                    driver->read<uint64_t>(lil2cpp_base + 0x2E47B8) + 0xB8) +
                                0x0) +
                            0x20) +
                        0x28) +
                    0x21C);
                ...
                //  读取内存原始毫秒级倒计时 → 增加3000ms（3秒）→ 避免负数 → 转换为秒级
                int 原始cd_ms = driver->read<int>(dataTable.pve[i].cd);
                int 原始maxcd_ms = driver->read<int>(dataTable.pve[i].maxcd);
                int 偏移后cd_ms = 原始cd_ms + 3000;
                int 偏移后maxcd_ms = 原始maxcd_ms + 3000;
                偏移后cd_ms = (偏移后cd_ms < 0) ? 0 : 偏移后cd_ms;
                偏移后maxcd_ms = (偏移后maxcd_ms < 0) ? 0 : 偏移后maxcd_ms;
                dataTable.pveTemp[i].cd = 偏移后cd_ms / 1000;
                dataTable.pveTemp[i].maxcd = 偏移后maxcd_ms / 1000;
                dataTable.pveTemp[i].id = driver->read<int>(dataTable.pve[i].id);
                if (dataTable.pveTemp[i].id == 0)
                {
                    continue;
                }
                ...
                bool 野怪已刷新 = (dataTable.pveTemp[i].cd <= 0);
                if (野怪已刷新)
                {
                    dataTable.pveTemp[i].coord.X = (float)固定坐标X;
                    dataTable.pveTemp[i].coord.Y = (float)固定坐标Y;
                    dataTable.pveTemp[i].hp = driver->read<int>(dataTable.pve[i].maxhp);
                ...
                if (shared)
                {
                    creeps += std::to_string(3) + "," + std::to_string(dataTable.pve[i].cd) + "," + std::to_string(70) + "," + std::to_string(pvegdX) + "," + std::to_string(pvegdY) + "==";
                }
        ...
        if (兵线)
        {
            ...
            long temp5 = ReadValue(libGame_base + 0x16C420);
            ...
            for (int l = 0; l < number; l++)
            {
                ...
                if (temp == foeComp)
                {
                    ...
                    dataTable.CeTemp[cont].coord.X = (float)driver->read<int>(
                        ReadValue(ReadValue(bxAddress + 0x230) + 0x60) + 0x0);
                    dataTable.CeTemp[cont].coord.Y = (float)driver->read<int>(
                        ReadValue(ReadValue(bxAddress + 0x230) + 0x60) + 0x8);
...
  if (绘制触摸)
    ...
    else if (ReadDword(libGame_base + 0x1DF82D4) == 0)
    {
        ...
        std::stringstream ss;
        ss << std::put_time(std::localtime(&now_c), "当前时间：%m月%d日 %H:%M:%S");

        // 添加时间信息到文本中
        std::string text = "等待对局加载\n";
        text += ss.str();
        const char *cstr = text.c_str();
        //
