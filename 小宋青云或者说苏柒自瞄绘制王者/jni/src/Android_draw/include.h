...
//static bool 十人 = false;
static int jmm=0;
static bool 自动解密 = false;
static bool 原坐标 = false;
static bool jm = false;
bool 貂蝉减cd = false;
...
int ReadDrawSet(总颜色 *myDrawSet, const char* Filename)
{
    ...
    if (pFile == NULL) {
        printf("\033[31;1m");//红色
        printf("[!] 文件打开失败: %s, 错误: %s\n", Filename, strerror(errno));
        return 0;
    }

 
    size_t read_count = fread(myDrawSet, sizeof(总颜色), 1, pFile);
    fclose(pFile); 
...
// 封装写入配置内容的函数（复用逻辑）
static void write_config_content(FILE* file) {
    if (file == NULL) return;
    
    // 所有配置项，两个文件会写入完全相同的内容
    fprintf(file, "地图=%d\n", 地图);
    fprintf(file, "射线=%d\n", 射线);
    fprintf(file, "视野方框=%d\n", 视野方框);
    fprintf(file, "方框=%d\n", 方框);
...
void write_config() {
    // 确保目录存在
    if (!ensure_directory("/data/小宋青云/")) {
        return;
    }

    // 配置文件路径数组（仅文件名不同）
    const char* config_files[] = {
        "/data/小宋青云/配置.ini颜色配置",
        "/data/小宋青云/配置.ini选项"
    };
    const int file_count = sizeof(config_files) / sizeof(config_files[0]);

    // 循环处理每个文件
    for (int i = 0; i < file_count; i++) {
        FILE* file = fopen(config_files[i], "w");
        if (file == NULL) {
            printf("\033[31;1m"); // 红色加粗
            perror("[!] 文件写入失败");
        ...
        // 写入相同的内容
        write_config_content(file);
        fclose(file);
    }
}

void read_config() {
    FILE *file = fopen("/data/小宋青云/配置.ini选项", "r");
    if (file == NULL) {
        printf("\033[31;1m");//红色
        perror("[!] 读取配置失败");
    ...
    if (fscanf(file, "地图=%d\n", &temp) == 1) 地图 = temp;
    if (fscanf(file, "射线=%d\n", &temp) == 1) 射线 = temp;
    if (fscanf(file, "视野方框=%d\n", &temp) == 1) 视野方框 = temp;
    if (fscanf(file, "方框=%d\n", &temp) == 1) 方框 = temp;
    if (fscanf(file, "野怪=%d\n", &temp) == 1) 野怪 = temp;
    if (fscanf(file, "兵线=%d\n", &temp) == 1) 兵线 = temp;
    if (fscanf(file, "方框技能=%d\n", &temp) == 1) 方框技能 = temp;
    if (fscanf(file, "顶上技能=%d\n", &temp) == 1) 顶上技能 = temp;
    if (fscanf(file, "自身=%d\n", &temp) == 1) 自身 = temp;
    if (fscanf(file, "头像常显=%d\n", &temp) == 1) 头像常显 = temp;
    if (fscanf(file, "小技能计时=%d\n", &temp) == 1) 小技能计时 = temp;
    if (fscanf(file, "暗化=%d\n", &temp) == 1) 暗化 = temp;
...
#include "Gerpicture/picture_obj550_png.h"
#include "Gerpicture/picture_obj584_png.h"
...
HeadPortrait[550] = createTexturePNGFromMem(picture_obj550_PNG_H, sizeof(picture_obj550_PNG_H));
HeadPortrait[584] = createTexturePNGFromMem(picture_obj584_PNG_H, sizeof(picture_obj584_PNG_H));
}
...
#include "Gerpicture/picture_obj55010_png.h"
#include "Gerpicture/picture_obj58410_png.h"
...
#include "Gerpicture/picture_obj55020_png.h"
#include "Gerpicture/picture_obj58420_png.h"
...
#include "Gerpicture/picture_obj55030_png.h"
#include "Gerpicture/picture_obj58430_png.h"
...
HeadPortrait[55030] = createTexturePNGFromMem(picture_obj55030_PNG_H, sizeof(picture_obj55030_PNG_H));
HeadPortrait[58430] = createTexturePNGFromMem(picture_obj58430_PNG_H, sizeof(picture_obj58430_PNG_H));
...
HeadPortrait[55020] = createTexturePNGFromMem(picture_obj55020_PNG_H, sizeof(picture_obj55020_PNG_H));
HeadPortrait[58420] = createTexturePNGFromMem(picture_obj58420_PNG_H, sizeof(picture_obj58420_PNG_H));
...
HeadPortrait[55010] = createTexturePNGFromMem(picture_obj55010_PNG_H, sizeof(picture_obj55010_PNG_H));
HeadPortrait[58410] = createTexturePNGFromMem(picture_obj58410_PNG_H, sizeof(picture_obj58410_PNG_H));
